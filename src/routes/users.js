...
router.post('/', authenticateToken, requireManager, (req, res) => {
    try {
        ...
        if (!['admin', 'manager', 'team_member', 'associate_partner', 'partner'].includes(role)) {
            return res.status(400).json({ error: 'Role must be admin, manager, partner, associate_partner, or team_member' });
        }
